...
class GeneratorDriver:
    ...
    def configure(self, frequency_hz: float, power_w: float) -> None:
        self.resource.write(f"SOUR:FREQ {frequency_hz}")
        self.resource.write(f"SOUR:POW {power_w}")
...
class PowerMeterDriver:
    ...
    def _measure(self, command: str) -> float:
        try:
            ...
            raise MalformedResponse("non-numeric instrument reply") from error

    def measure_forward_power_w(self) -> float:
        return self._measure("MEAS:POW?")

    def measure_reflected_power_w(self) -> float:
        return self._measure("MEAS:REFL?")
