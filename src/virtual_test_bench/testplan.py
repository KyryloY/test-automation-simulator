...
def _limits(value: Any, name: str) -> Limits:
    ...
    lower = (
        _number(value["lower"], f"limits.{name}.lower")
        if value.get("lower") is not None
        else None
    )
    upper = (
        _number(value["upper"], f"limits.{name}.upper")
...
def parse_plan(mapping: dict[str, object]) -> TestPlan:
    try:
        ...
        if readings <= 0 or isinstance(mapping["readings"], bool):
            raise ValidationError("readings must be positive")
        if retries < 0 or isinstance(mapping["retries"], bool) or guard_band_w < 0:
            raise ValidationError("retries and guard_band_w must be non-negative")
        return TestPlan(
            str(mapping["id"]),
            _number(mapping["frequency_hz"], "frequency_hz", positive=True),
            _number(mapping["power_setpoint_w"], "power_setpoint_w", positive=True),
            readings,
