...
def forward(result):
    return next(item for item in result.measurements if item.quantity == "forward_power")


def test_nominal_plan_passes():
    assert {item.verdict for item in run_plan(load_plan(PLAN), 42, "nominal").measurements} == {Verdict.PASS}
...
def test_timeout_is_inconclusive_after_retry():
    assert forward(run_plan(load_plan(PLAN), 42, "meter_timeout")).verdict is Verdict.INCONCLUSIVE
